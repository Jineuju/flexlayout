// 1단계 : 변수에 js로 변화/동작시킬 요소를 담아야합니다.
...
// 각각의 article 개별로 이벤트를 걸어주셔야 합니다

// 반복문을 쓰지 않는다면 인덱스별로 이벤트를 하나하나 붙여서 작업해야한다
// 코드가 길어지고 불필요하기에 반복문을 작성한다
// articles[0].addEventListener("mouseenter", function (e) {
//     e.currentTarget.querySelector("video").play();
// });
// 만약 3개의 아티클이면 위의 문장을 3번 써야하고
// 추가해서 3개의 아티클에 4개의 요소가 있다면
// 12번 써야한다

for (let el of articles) {
  // 마우스가 들어가면 재생되는 이벤트
  // 호버를 js로 작성하면 두가지 이벤트로 작성해야한다
  // mouseenter, mouseleave 이벤트 두가지를 다 걸어야
  // 호버처럼 작동합니다.
  ...
  /*
  currentTarget은 이벤트 리스너가 붙어지는
  그 대상을 의미합니다.
  지금 상황에서는 마우스를 엔터한 아티클을 의미합니다.
  만약 3인덱스 아티클에 마우스를 올렸다면
  currentTarget은 3인덱스 아티클이 됩니다.
  */

  /*
  이벤트 객체 : js에서 이벤트가 발생 시 이벤트객체에 해당정보가
  발생합니다.
        el.addEventListener("click", function (e) {
        (e)의 e가 이벤트 객체를 매개변수로 가져온것으로
        해당 이벤트의 정보를 함수안으로 값을 가져오게됩니다.
  제어문들에서는 특정값을 정확히 참조하는 것이 아닌 ~할 때 이므로
  이벤트객체를 참조해야만 정확하게 어떤 요소에서 이벤트가 일어나서
  그 요소를 참조해야하는지를 알 수 있습니다.
  */
...
// 닫기버튼
// 닫기버튼은 정확하게 참조되기에 이벤트객체를 참조하지 않습니다
